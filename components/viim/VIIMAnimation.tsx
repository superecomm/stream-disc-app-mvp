...
export default function VIIMAnimation({ 
  ...
  useEffect(() => {
    ...
    if (visualStyle === "particles") {
      ...
      if (recordingCountdown && recordingCountdown > 0) {
        ...
        // Processing: Compact swirling node (ChatGPT "thinking" style)
        particleCount = 8;
        initParticles = Array(particleCount).fill(0).map((_, i) => {
          const angle = (i / particleCount) * Math.PI * 2;
          const radius = 20;
    ...
    const animate = () => {
      ...
      if (visualStyle === "canvas" && canvasRef.current) {
        ...
        if (ctx) {
          ...
          const layers = state === "speaking" ? 5 : 3;
          for (let layer = 0; layer < layers; layer++) {
            ...
            for (let x = 0; x < canvas.width; x += 2) {
              ...
              const intensity = state === "speaking" ? 1 : 0.3;
            ...
            ctx.strokeStyle = `hsla(${hue}, 80%, 60%, ${state === "speaking" ? 0.8 : 0.4})`;
            ctx.stroke();
          }
        }
      }
      ...
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        {Array(layers).fill(0).map((_, layerIndex) => {
          ...
          for (let i = 0; i <= numPoints; i++) {
            ...
            const y = height / 2 + waveValue * (state === "speaking" ? 40 : 15) + layerOffset;
            points.push(`${x},${y}`);
          }
          ...
          return (
            <polyline
              ...
              opacity={state === "speaking" ? 0.8 : 0.5}
              style={{
                filter: `drop-shadow(0 0 ${state === "speaking" ? '8px' : '4px'} ${color})`
              }}
            />
          );
        })}
  ...
  const renderBlob = () => {
    ...
    for (let i = 0; i < points; i++) {
      ...
      const baseRadius = state === "speaking" ? size * 0.4 : size * 0.25;
    ...
    return (
      <svg width={size * 2} height={size}>
        ...
        <path
          ...
          opacity={state === "speaking" ? 0.9 : 0.5}
          style={{
            filter: `drop-shadow(0 0 ${state === "speaking" ? '20px' : '10px'} #8b5cf6)`
          }}
        />
      </svg>
    );
    ...
    return (
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        {bars.slice(0, segments).map((amplitude, index) => {
          ...
          const waveHeight = amplitude * (state === "speaking" ? 80 : 30);
      ...
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        {particles.map((particle, index) => {
          ...
          const explosionGlow = isExploding && explosionProgress < 0.4
            ...
            : particleRadius * (state === "speaking" ? 3 : 2);
          ...
          return (
            <g key={index}>
              ...
              <circle
                ...
                style={{
                  filter: `drop-shadow(0 0 ${state === "speaking" || isExploding ? '8px' : state === "listening" ? '4px' : '3px'} ${isExploding && explosionProgress < 0.4 ? 'rgba(255, 255, 255, 1)' : color})`
                }}
              />
            </g>
          );
  ...
  const containerShape = container === "circle" ? "rounded-full" : "rounded-sm";
  const backgroundShape = container === "circle" ? "rounded-full" : "rounded-sm";
      ...
      >
        ...
        <div 
          ...
          style={{
            ...
            animation: (state === "speaking" || state === "recording") ? "spin-smooth 15s ease-in-out infinite" : "none"
          }}
        />
        ...
        <div 
          ...
          style={{
            ...
            animation: (state === "speaking" || state === "recording") ? "spin-smooth 15s ease-in-out infinite reverse" : "none"
          }}
        />
        ...
        >
          ...
          {state === "speaking" && (
            <div 
              ...
              style={{
                borderRadius: container === "circle" ? "9999px" : "0.125rem"
              }}
            >
              <div
