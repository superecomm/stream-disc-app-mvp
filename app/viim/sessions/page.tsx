...
import type { ViimSession, ChatMessage as ChatMessageType } from "@/types/viim";

export default function ChatSessionsPage() {
  const { currentUser } = useAuth();
  const [sessions, setSessions] = useState<ViimSession[]>([]);
  const [selectedSession, setSelectedSession] = useState<ViimSession | null>(null);
  ...
  const fetchSessions = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/viim/sessions?userId=${currentUser.uid}`);
      if (response.ok) {
        const data = await response.json();
        // Filter to only show sessions with chat messages
        const chatSessions = (data.sessions || []).filter(
          (session: ViimSession) => session.chatMessages && session.chatMessages.length > 0
        );
  ...
  const getSessionTitle = (session: ViimSession) => {
